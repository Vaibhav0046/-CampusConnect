...
public class FacultyManagementController {
    ...
    public String dashboard(HttpSession session, Model model) {
        ...
        Faculty faculty = (Faculty) userRepository.findById(facultyId).orElseThrow(() -> new RuntimeException("Faculty not found"));

        // 1. Fetch MCQs (created by this faculty or generally)
        List<Question> questions = questionRepository.findAll();

        // 2. Fetch Coding challenges
        List<CodingQuestion> codingQuestions = codingQuestionRepository.findAll();

        // 3. Fetch scheduled assessments
        List<Test> scheduledTests = testRepository.findAll();

        // 4. Student Roster with platforms telemetry
        ...
        for (User s : students) {
            ...
            UserCodingProfile profile = userCodingProfileRepository.findByUserId(s.getId()).orElse(null);
            if (profile != null) {
            ...
            long solvedCount = studentCodingStatusRepository.countByStudentIdAndStatus(s.getId(), "SOLVED");
    ...
    public String createQuestion(
            ...
            @RequestParam(value = "marks", required = false, defaultValue = "1") int marks,
            ...
            @RequestParam(value = "correctOption", required = false) String correctOption,
            // Coding Question fields
            @RequestParam(value = "title", required = false) String title,
            @RequestParam(value = "platform", required = false) String platform,
            @RequestParam(value = "problemLink", required = false) String problemLink,
            @RequestParam(value = "score", required = false, defaultValue = "10") int score,
            HttpSession session) {

        User facultyUser = (User) session.getAttribute("user");
        if (facultyUser == null) return "redirect:/";

        Faculty faculty = (Faculty) userRepository.findById(facultyUser.getId()).orElseThrow(() -> new RuntimeException("Faculty not found"));
    ...
    public String deleteQuestion(
        ...
        if (faculty == null) return "redirect:/";
    ...
    public ResponseEntity<byte[]> exportCSV() {
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=questions_Java.csv")
    ...
    public String scheduleTest(
            ...
            @RequestParam(value = "questionIds", required = false) Set<Long> questionIds,
            @RequestParam(value = "assignedStudentIds", required = false) Set<Long> assignedStudentIds,
            HttpSession session) {
        ...
        dto.setFacultyId(faculty.getId());
        dto.setQuestionIds(questionIds != null ? questionIds : Collections.emptySet());
        dto.setAssignedStudentIds(assignedStudentIds != null ? assignedStudentIds : Collections.emptySet());
